...
namespace Autohand
{
    ...
    public class PlacePoint : MonoBehaviour
    {
        ...
        [Tooltip("Whether or not this place point is in close proximity ")]
        public bool closeProximityPlacePoint = false;
        [Space]
        ...
        public PlacePointEvent OnNonPlacableObjectHoveredEvent;
        public PlacePointEvent OnStopNonPlacableHighlightEvent;

        public Grabbable highlightingObj { get; protected set; } = null;
        //adding non placacble obj ref here.
        public Grabbable nonPlacablehighlightingObj { get; protected set; } = null;
        ...
        public bool runTimeCustomDebug = false;



        public AudioClip placeSound;
        private AudioSource _placeAudioSource;
        ...
        void SetupPlaceSound()
        {
            if (placeSound != null)
            {
                _placeAudioSource = gameObject.AddComponent<AudioSource>();
                _placeAudioSource.playOnAwake = false;
                _placeAudioSource.clip = placeSound;
            }
        }
        ...
        Grabbable tempNonPlacableGrabbable;
        ...
        void CheckPlaceObject(float radius, float scale)
        {
            ...
            if (!disablePlacePointOnPlace && !disableRigidbodyOnPlace && placedObject != null && !IsStillOverlapping(placedObject, scale) && !closeProximityPlacePoint)
            {
                Debug.Log("disablePlacePointOnPlace : " + disablePlacePointOnPlace + " IsStillOverlapping(placedObject, scale) : " + IsStillOverlapping(placedObject, scale));
                Remove(placedObject);
            }


            if (placedObject == null && highlightingObj == null && nonPlacablehighlightingObj == null)
            {
                ...
                if (debugSphere != null)
                    Destroy(debugSphere);
                if (runTimeCustomDebug)
                {
                    debugSphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                    debugSphere.transform.position = placedOffset.position + placedOffset.rotation * radiusOffset;
                    debugSphere.transform.localScale = new Vector3(placeRadius * scale, placeRadius * scale, placeRadius * scale);
                    debugSphere.transform.parent = transform;
                    debugSphere.GetComponent<Collider>().isTrigger = true;
                }
                ...
                if (overlaps != lastOverlapCount)
                {
                    for (int i = 0; i < overlaps; i++)
                    {
                        ...
                        else if (AutoHandExtensions.HasGrabbable(collidersNonAlloc[i].gameObject, out tempNonPlacableGrabbable) && (tempNonPlacableGrabbable != nonPlacablehighlightingObj) && (CanPlace(tempNonPlacableGrabbable) == false))
                        {
                            NonPlacabaleHighlight(tempNonPlacableGrabbable);
                            break;
                        }
                    }
                }
            ...
            if (nonPlacablehighlightingObj != null)
            {
                if (!IsStillOverlapping(nonPlacablehighlightingObj, scale))
                {
                    StopNonPlacableHighlight(nonPlacablehighlightingObj);
                }
            }
        }
        ...
        private void PlayPlaceSound()
        {
            ...
            if (_placeAudioSource != null && _placeAudioSource.clip != null)
            {
                _placeAudioSource.Play();
            }
        }
        ...
        {
            Debug.Log("Remove method is getting called");
            Debug.Log("Tmp Required value for parent on grab : false, current value is: " + placeObj.parentOnGrab + " parent on place : true, current value is: " + parentOnPlace + " gameobject.activeInHierarchy: true, current value is : " + gameObject.activeInHierarchy);
            ...
            Debug.Log("Required value for parent on grab : false, current value is: " + placeObj.parentOnGrab + " parent on place : true, current value is: " + parentOnPlace + " gameobject.activeInHierarchy: true, current value is : " + gameObject.activeInHierarchy);
            if (!placeObj.parentOnGrab && parentOnPlace && gameObject.activeInHierarchy)
            {
                Debug.Log("Setting to original parent!");
        ...
        internal virtual void NonPlacabaleHighlight(Grabbable from)
        {
            if (nonPlacablehighlightingObj == null)
            {

                nonPlacablehighlightingObj = from;
                //nonPlacablehighlightingObj.OnPlacePointHighlightEvent?.Invoke(this, highlightingObj);
                /*foreach (var grabChild in highlightingObj.grabbableChildren)
                    grabChild.OnPlacePointHighlightEvent?.Invoke(this, grabChild);
                foreach (var garb in highlightingObj.grabbableParents)
                    garb.OnPlacePointHighlightEvent?.Invoke(this, garb);*/

                Debug.Log("On Non Placable Object Hovered invoked!", gameObject);
                OnNonPlacableObjectHoveredEvent?.Invoke(this, from);
            }
        }
        ...
        internal virtual void StopNonPlacableHighlight(Grabbable from)
        {
            if (nonPlacablehighlightingObj != null)
            {
                nonPlacablehighlightingObj = null;
                Debug.Log("On Non Placable Object UnHovered invoked!");
                OnStopNonPlacableHighlightEvent?.Invoke(this, from);
            }
        }
        ...
        {
            //Debug.Log("placeLayers : " + placeLayers + " collidersNonAlloc : " + collidersNonAlloc);
            ...
            {
                //Debug.Log("IsStillOverlapping : " + (collidersNonAlloc[i].attachedRigidbody == from.body));
