...
namespace Autohand
{
    ...
    public class Hand : HandBase
    {
        ...
        [AutoToggleHeader("Enable Hovering", 0, 0, tooltip = "Raycasting for grabbables to hover you can disable it here if you aren't using it")]
        public bool usingHovering = true;

        [EnableIf("usingHovering")]
        [Tooltip("The layers to highlight and use look assist on --- Nothing will default on start")]
        public LayerMask hoveringLayers;
        ...
        public static string pointInteractableLayerName = "Interactable";
        ...
        /// <summary>Called when highlighting starts</summary>
        public event HandGrabEvent OnHoverStarted;

        /// <summary>Called when highlighting stops</summary>
        public event HandGrabEvent OnHoverStopped;
        ...
        private SkinnedMeshRenderer handSkinMeshRenderer;
        ...
        protected override void Awake()
        {
            ...
            if (hoveringLayers.value == 0 || hoveringLayers == LayerMask.GetMask(""))
            {
                hoveringLayers = LayerMask.GetMask(grabbableLayerNameDefault);
            }
            ...
            handSkinMeshRenderer = gameObject.GetComponentInChildren<SkinnedMeshRenderer>();
            if (handSkinMeshRenderer)
            {
                OriginalHandMaterial = handSkinMeshRenderer.material;
            }
        }
        ...
        protected override void OnEnable()
        {
            ...
            hoverRoutine = StartCoroutine(HovertUpdate(1 / 30f));
        ...
        public virtual void Grab(GrabType grabType)
        {
            ...
            if (usingHighlight && !grabbing && holdingObj == null && lookingAtObj != null)
            {
                ...
                GrabbableWrapper grabbableW = hoveringAtObj.GetComponent<GrabbableWrapper>();
                if (grabbableW != null)
                {
                    if (grabbableW.CanBeGrabbed())
                    {
            ...
            else if (usingHovering && !grabbing && holdingObj == null && hoveringAtObj != null)
            {
                var newGrabType = this.grabType;
                ...
                if (grabbableW != null)
                {
                    if (grabbableW.CanBeGrabbed())
                    {
                        ...
                        if (hoveringAtObj.grabType != HandGrabType.Default)
                        {
                            newGrabType = hoveringAtObj.grabType == HandGrabType.GrabbableToHand ? GrabType.GrabbableToHand : GrabType.HandToGrabbable;
                            Debug.Log("Grab type for this grabbable is:" + newGrabType, gameObject);
                        }
                        grabRoutine = StartCoroutine(GrabObject(GetHoverHit(), hoveringAtObj, newGrabType));//this is changed.
                    }
                    else
                    {
                        grabbableW.onGrabbedAttemptEvent?.Invoke();
                        //Attempting a failed grab, perfect place for the event
                        Debug.LogWarning("No Object to Grab!!");//this where the hand is acting weird, not true everytime!!
                    }
                }
                else
                {
                    Debug.Log("Entered the else block of grabbable wrapper null!");
                    if (HandClosestHitForHovering(out RaycastHit closestHit, out Grabbable grabbable, reachDistance, ~handLayers) && grabbable != null)
                    {
                        Debug.Log("Entered the if block of grabbable wrapper null!");
                        var newGrabTypeTmp = this.grabType;
                        GrabbableWrapper grabbableWr = grabbable.GetComponent<GrabbableWrapper>();
                        if (grabbableWr != null && grabbableWr.CanBeGrabbed())
                        {
                            if (grabbable.grabType != HandGrabType.Default)
                                newGrabTypeTmp = grabbable.grabType == HandGrabType.GrabbableToHand ? GrabType.GrabbableToHand : GrabType.HandToGrabbable;
                            if (grabbable != null)
                            {
                                Debug.Log("GrabRoutine is being called from else of using hovering!");
                                grabRoutine = StartCoroutine(GrabObject(closestHit, grabbable, newGrabTypeTmp));
                            }
                        }
                        else
                        {
                            grabbable.onGrabFailed?.Invoke(this, grabbable);
                            // grabbableW.onGrabbedAttemptEvent?.Invoke();
                            Debug.Log("Too far from object to grab");
                        }
                    }
                }
            }
            else if (!grabbing && holdingObj == null)
            {
                ...
                {
                    var newGrabType = this.grabType;
                    GrabbableWrapper grabbableW = grabbable.GetComponent<GrabbableWrapper>();
                    if (grabbableW != null && grabbableW.CanBeGrabbed())
                    {
                        if (grabbable.grabType != HandGrabType.Default)
                            newGrabType = grabbable.grabType == HandGrabType.GrabbableToHand ? GrabType.GrabbableToHand : GrabType.HandToGrabbable;
                        if (grabbable != null)
                        {
                            Debug.Log("GrabRoutine is being called from last else if hand is holding nothing!");
        ...
        Collider[] hoverCollidersNonAlloc = new Collider[128];
        List<Grabbable> foundGrabbablesForHover = new List<Grabbable>();
        ...
        /// <summary>Manages the hovering for grabbables</summary>
        public virtual void UpdateHovering()
        {
            if (usingHovering)
            {
                RaycastHit hit;
                HandClosestHitForHovering(out hoverRaycastHit, out Grabbable newHoveringAtObj, hoverDistance, ~handLayers);

                if (newHoveringAtObj != null)
                {
                    GrabbableWrapper hoveringAtObjGrabbableWrapper = newHoveringAtObj.GetComponent<GrabbableWrapper>();
                    //if (newHoveringAtObj.CanGrab(this)) Can be used when the grabbable is grabbable = false
                    {
                        //Changes look target
                        if (newHoveringAtObj != hoveringAtObj)
                        {
                            //Unhighlights current target if found
                            if (hoveringAtObj != null)
                            {
                                OnHoverStopped?.Invoke(this, hoveringAtObj);
                                if (hoveringAtObjGrabbableWrapper != null)
                                    hoveringAtObjGrabbableWrapper.onGrabbableHoveredStoppedEvent?.Invoke(this, hoveringAtObj);
                            }
                            hoveringAtObj = newHoveringAtObj;
                            //lookingAtObj.gameObject.TryGetComponent<GrabbableWrapper>(out hoveringAtObjGrabbableWrapper);
                            //Debug.Log("Hovering over Obj" + hoveringAtObj.name);
                            //hoverRaycastHit = hit;
                            //Highlights new target if found
                            OnHoverStarted?.Invoke(this, hoveringAtObj);
                            if (hoveringAtObjGrabbableWrapper != null)
                                hoveringAtObjGrabbableWrapper.onGrabbableHoveredStartedEvent?.Invoke(this, hoveringAtObj);
                        }
                    }
                }
                //If it was looking at something but now it's not there anymore
                else if (newHoveringAtObj == null && hoveringAtObj != null)
                {
                    GrabbableWrapper hoveringAtObjGrabbableWrapper = hoveringAtObj.GetComponent<GrabbableWrapper>();
                    //Just in case the object your hand is looking at is destroyed
                    OnHoverStopped?.Invoke(this, hoveringAtObj);
                    if (hoveringAtObjGrabbableWrapper != null)
                        hoveringAtObjGrabbableWrapper.onGrabbableHoveredStoppedEvent?.Invoke(this, hoveringAtObj);
                    ...
                    hoveringAtObj = null;
                    hoveringAtObjGrabbableWrapper = null;
                }

                else if (hoveringAtObj != null)
                {
                    GrabbableWrapper hoveringAtObjGrabbableWrapper = hoveringAtObj.GetComponent<GrabbableWrapper>();
                    //Just in case the object your hand is looking at is destroyed
                    OnHoverStopped?.Invoke(this, hoveringAtObj);
                    hoveringAtObj.Unhighlight(this);
                    if (hoveringAtObjGrabbableWrapper != null)
                        hoveringAtObjGrabbableWrapper.onGrabbableHoveredStoppedEvent?.Invoke(this, hoveringAtObj);
                    hoveringAtObj = null;
                    hoveringAtObjGrabbableWrapper = null;
        ...
        /// <summary>Returns the closest raycast hit from the hand's hovering system, if no hover, returns blank raycasthit</summary>
        public RaycastHit GetHoverHit()
        {
            hoverRaycastHit.point = handGrabPoint.position;
            hoverRaycastHit.normal = handGrabPoint.up;
            return hoverRaycastHit;
        }
        //======================== GETTERS AND SETTERS ====================
        //=================================================================
        //=================================================================
        ...
        //Hovering doesn't need to be called every update, it can be called every 4th update without causing any noticable differrences 
        IEnumerator HovertUpdate(float timestep)
        {
            //This will smooth out the highlight calls to help prevent lag spikes
            if (left)
                yield return new WaitForSecondsRealtime(timestep / 2);
        ...
        #region public methods added for VRB

        public void WearGloves()
        {
            if (handSkinMeshRenderer && GlovesMaterial)
            {
                handSkinMeshRenderer.material = GlovesMaterial;
            }
        }
        public void RemoveGloves()
        {
            if (handSkinMeshRenderer)
            {
                handSkinMeshRenderer.material = OriginalHandMaterial;
            }
        }
