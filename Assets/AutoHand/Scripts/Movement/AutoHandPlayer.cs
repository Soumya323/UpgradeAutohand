...
namespace Autohand
{
    ...
    public class AutoHandPlayer : MonoBehaviour
    {
        ...
        void SyncBodyHead()
        {
            ...
            var flatHeadPos = headCamera.transform.position;
            flatHeadPos.y = 0;
            var flatBodyPos = transform.position;
            flatBodyPos.y = 0;
            ...
            if (rotationType == RotationType.snap)
            {
                if (Mathf.Abs(turningAxis) > turnDeadzone && axisReset)
                {
                    ...
                    if (headPhysicsFollower != null)
                    {
                        ...
                        Debug.Log("Snapped to tile correctly");
                    }
        ...
        public virtual void SetPosition(Vector3 position, Quaternion rotation)
        {
            ...
            transform.position += deltaPos;
            //This code will move the tracking objects to match the body collider position when moving
            var targetPos = transform.position - trackingContainer.transform.position; // changed headCamera to trackingContainer
            ...
            //removed headCamera
            var deltaRot = rotation * Quaternion.Inverse(trackingContainer.transform.rotation);  // changed headCamera to trackingContainer
            trackingContainer.RotateAround(trackingContainer.transform.position, Vector3.up, deltaRot.eulerAngles.y);  // changed headCamera to trackingContainer
            Recenter();
        }
        ...
        public virtual void SetRotation(Quaternion rotation)
        {
            ...
            trackingContainer.position += targetPos;
            Debug.Log("rotated the player correctly");
            if (headPhysicsFollower != null)
            {
                headPhysicsFollower.transform.position += targetPos;
                headPhysicsFollower.body.position = headPhysicsFollower.transform.position;
        ...
        {
            Debug.Log(transform.position + "and" + headCamera.transform.position);
            var targetPos = transform.position - headCamera.transform.position;

            targetPos.y = 0;

            trackingContainer.position += targetPos;
            Debug.Log(targetPos + "and" + trackingContainer.position);
            if (headPhysicsFollower != null)
            {
                headPhysicsFollower.transform.position += targetPos;
            ...
            targetPosOffset = Vector3.zero;
            targetTrackedPos = new Vector3(trackingContainer.position.x, targetTrackedPos.y, trackingContainer.position.z);
            Debug.Log("player recentered correctly");
            UpdateTrackedObjects();
