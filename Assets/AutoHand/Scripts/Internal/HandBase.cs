...
namespace Autohand {
    ...
    public delegate void HandInteractableEvent(Hand hand, PointInteractable point);
    [Serializable] public class UnityInteractableEvent : UnityEvent<Hand, PointInteractable> { }
    ...
    public class HandBase : MonoBehaviour {
        ...
        [Tooltip("Maximum distance for Hovering"), Min(0.01f)]
        ...
        [AutoToggleHeader("Hovering Debug", 0, 0, tooltip = "Whether or not to enable the hand's Hovering debug")]
        public bool hoverDebugEnabled = false;
        [SerializeField] private LineRenderer lineRendererDebugForHover;
        private GameObject hoverDebugSphere;
        ...
        Grabbable _hoveringAtObj = null;
        public Grabbable hoveringAtObj {
            get { return _hoveringAtObj; }
            protected set { _hoveringAtObj = value; }
        }
        ...
        protected RaycastHit hoverRaycastHit;
        ...
        protected virtual void Awake() {
            ...
            {
                lineRendererDebugForHover = transform.Find("LineRenderer")?.GetComponent<LineRenderer>();
            }
            ...
            _playerRightHand = _playerCamera.transform.parent.Find("Controller (right)").gameObject;
            _playerLeftHand = _playerCamera.transform.parent.Find("Controller (left)").gameObject;
    }
        ...
        Collider[] handHoverNonAlloc = new Collider[128];
        ...
        protected virtual bool HandClosestHitForHovering(out RaycastHit closestHit, out Grabbable grabbable, float dist, int layerMask, Grabbable target = null)
        {
            Vector3 palmForwardH = palmTransform.forward;
            Vector3 palmPositionH = palmTransform.position;
            Collider col;

            Grabbable currentGrabbable = null;
            Grabbable closestGrabbable = null;
            float currentGrabbableDistance = 0f;
            float closestGrabbableDistance = 0f;
            Collider closestGrabbableCollider = null;
            RaycastHit hit;
            Ray ray = new Ray();

            //bool isHitting = Physics.Raycast(palmPositionH, palmForwardH, out hit, hoverDistance, layerMask, queryTriggerInteraction: QueryTriggerInteraction.Ignore);
            RaycastHit[] hits = Physics.SphereCastAll(palmPositionH, 0.4f, palmForwardH, hoverDistance, layerMask, queryTriggerInteraction: QueryTriggerInteraction.Ignore);
            //if(hit.collider != null) Debug.Log(hit.collider.gameObject.name);
            if (hoverDebugEnabled && lineRendererDebugForHover != null)
            {
                if(lineRendererDebugForHover == null)
                {
                    lineRendererDebugForHover = transform.Find("LineRenderer")?.GetComponent<LineRenderer>();
                }
                lineRendererDebugForHover.SetWidth(0f, 0f);
            }
            if(hits.Length > 0)
            {
                ...
                {
                    col = hits[i].collider;
                    if (col.gameObject.HasGrabbable(out currentGrabbable))
                    {
                        currentGrabbableDistance = Vector3.Distance(palmPositionH, col.ClosestPoint(palmPositionH));
                        if (closestGrabbable == null)
                        {
                            closestGrabbable = currentGrabbable;
                            closestGrabbableDistance = currentGrabbableDistance;
                            closestGrabbableCollider = col;
                        }
                        else if (currentGrabbableDistance < closestGrabbableDistance)
                        {
                            closestGrabbable = currentGrabbable;
                            closestGrabbableDistance = currentGrabbableDistance;
                            closestGrabbableCollider = col;
                ...
                if (closestGrabbable != null)
                {
                    if (!(closestGrabbableCollider is MeshCollider) || (closestGrabbableCollider as MeshCollider).convex == true)
                    {
                        Vector3 closestPoint = closestGrabbableCollider.ClosestPoint(palmTransform.transform.position);
                        ray.direction = closestPoint - palmTransform.position;
                    }
                    else
                    {
                    ...
                    ray.origin = palmTransform.transform.position;
                    ray.origin = Vector3.MoveTowards(ray.origin, closestGrabbableCollider.bounds.center, 0.001f);
                    RaycastHit checkhit;
                    if (Physics.Raycast(ray, out checkhit, hoverDistance, layerMask, queryTriggerInteraction: QueryTriggerInteraction.Ignore))
                    {
                        closestHit = checkhit;
                        ...
                        if (closestGrabbable != null)
                        {
                            if (hoverDebugEnabled)
                            {
                                ...
                                if (hoverDebugSphere == null)
                                {
                                    hoverDebugSphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                                    hoverDebugSphere.GetComponent<Renderer>().material.color = Color.cyan;
                                }
                                hoverDebugSphere.transform.position = closestHit.point;
                                hoverDebugSphere.GetComponent<SphereCollider>().isTrigger = true;
                                hoverDebugSphere.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f);
                            }
                            else
                            {
                                if (hoverDebugSphere != null)
                                {
                                    Destroy(hoverDebugSphere);
                                }
                            }
                            return true;
                        }
            ...
            if (isHitting)
            {
                int overlapCount = Physics.OverlapSphereNonAlloc(hit.point, checkSphereRadius, handHoverNonAlloc, layerMask, QueryTriggerInteraction.Ignore);

                if (hoverDebugEnabled && lineRendererDebugForHover != null)
                {
                    lineRendererDebugForHover.SetWidth(0.01f, 0.03f);
                    lineRendererDebugForHover.startColor = new Color(0.3f, 0.8f, 0.9f, 0.2f);
                    lineRendererDebugForHover.SetPosition(0, palmPositionH);
                    lineRendererDebugForHover.SetPosition(1, hit.point);
                }

                for (int i = 0; i < overlapCount; i++)
                {
                    col = handHoverNonAlloc[i];
                    if (col.gameObject.HasGrabbable(out currentGrabbable))
                    {
                        currentGrabbableDistance = Vector3.Distance(palmPositionH, col.ClosestPoint(palmPositionH));
                        if (closestGrabbable == null)
                ...
                if (closestGrabbable != null)
                {
                    Debug.Log(closestGrabbableCollider);
                    if (!(closestGrabbableCollider is MeshCollider) || (closestGrabbableCollider as MeshCollider).convex == true)
                    {
                        Vector3 closestPoint = closestGrabbableCollider.ClosestPoint(palmTransform.transform.position);
                        ray.direction = closestPoint - palmTransform.position;
        ...
        private void OnDrawGizmos()
        {
            ...
            //Gizmos.DrawWireSphere(palmTransform.position + palmTransform.forward * (highlightDistance * 0.9f), checkSphereRadius);
            Gizmos.DrawWireSphere(hoverRaycastHit.point, checkSphereRadius);
        }
